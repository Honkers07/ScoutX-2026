...
const defaultData = {
  ...
  redTotalScore: 0,
  blueTotalScore: 0,
  redOfficialScore: null,
  blueOfficialScore: null,
  notes: "",
  timestamp: null,
};
...
export default class VideoScoutData {
  ...
  async submit() {
    ...
    if (isIncomplete) {
      this.sendAlert(
        "Incomplete Form - Please fill in all required fields and process both alliance videos",
        "error"
      );
      return false;
    }
    ...
    let firebaseData = {
      ...
      redOfficialScore: this.data.redOfficialScore || null,
      blueOfficialScore: this.data.blueOfficialScore || null,
      notes: this.data.notes || "",
      timestamp: Date.now(),
    };

    try {
      await setDoc(doc(db, "videoScoreData", this.data.match), firebaseData);
      return true;
    } catch (error) {
