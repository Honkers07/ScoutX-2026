import { Box, Typography, Stack, Button, Slider, Paper, ToggleButton, ToggleButtonGroup } from "@mui/material";
...
export default function VSVideoCrop(props) {
   ...
   const initialHeight = (initialWidth * ASPECT_RATIO_HEIGHT) / ASPECT_RATIO_WIDTH;
   ...
   const handleMouseMove = (e) => {
       ...
       const deltaX = ((currentX - dragStart.x) / canvasScale / videoDimensions.width) * 100;
       const deltaY = ((currentY - dragStart.y) / canvasScale / videoDimensions.height) * 100;
   ...
   const handleXChange = (event, newValue) => {
       setCropRegion(prev => ({
           ...prev,
           x: Math.min(newValue, 100 - prev.width),
       }));
   };
   ...
   const handleYChange = (event, newValue) => {
       setCropRegion(prev => ({
           ...prev,
           y: Math.min(newValue, 100 - prev.height),
       }));
   };
   ...
   const handleAspectRatioToggle = () => {
       setAspectRatioLocked(prev => {
           if (!prev) {
               ...
               setCropRegion(crop => ({
                   ...crop,
                   height: Math.min((crop.width * ASPECT_RATIO_HEIGHT) / ASPECT_RATIO_WIDTH, 100),
               }));
           }
           return !prev;
       });
   ...
   const maxWidthForAspectRatio = Math.floor((100 * ASPECT_RATIO_WIDTH) / ASPECT_RATIO_HEIGHT);


   const handleConfirm = () => {
       // Validate video dimensions before calculating crop
       if (videoDimensions.width === 0 || videoDimensions.height === 0) {
           console.error('[VSVideoCrop] Video dimensions not available - video may not be loaded');
           return;
       }
       ...
       <Box sx={{ width: "100%", mt: 2 }}>
           ...
           <Box sx={{ mt: 3, px: 1 }}>
               <Typography variant="body2" color="white" gutterBottom>
                   Horizontal Position (X): {cropRegion.x.toFixed(0)}%
               </Typography>
               <Slider
                   value={cropRegion.x}
                   onChange={handleXChange}
                   min={0}
                   max={100 - cropRegion.width}
                   sx={{ color: allianceColor }}
               />


               <Typography variant="body2" color="white" gutterBottom sx={{ mt: 2 }}>
                   Vertical Position (Y): {cropRegion.y.toFixed(0)}%
               </Typography>
               <Slider
                   value={cropRegion.y}
           ...
           {/* Aspect ratio toggle */}
           <Box sx={{ mt: 3, px: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
               <Typography variant="body2" color="white">
                   Aspect Ratio: {aspectRatioLocked ? `${ASPECT_RATIO_WIDTH}:${ASPECT_RATIO_HEIGHT} (locked)` : 'Unlocked'}
               </Typography>
               <ToggleButtonGroup
           ...
           <Box sx={{ mt: 2, px: 1 }}>
               {aspectRatioLocked ? (
                   <>
                       <Typography variant="body2" color="white" gutterBottom>
                           Crop Size: {cropRegion.width.toFixed(0)}% × {cropRegion.height.toFixed(0)}%
                       </Typography>
                       <Slider
           ...
           <Stack direction="row" spacing={2} sx={{ mt: 3 }}>
               ...
               <Button
                   ...
                   fullWidth
                   sx={{ backgroundColor: allianceColor, '&:hover': { backgroundColor: allianceColor } }}
               >
                   Confirm Crop
               </Button>
           </Stack>
