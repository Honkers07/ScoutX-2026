...
export default function VSAllianceTab(props) {
  ...
  const [stage, setStageLocal] = useState(
    stageProp !== undefined ? stageProp : AllianceStage.UPLOAD
  );
  ...
  const handleCropConfirm = (cropData) => {
    // cropData now includes: x, y, width, height, startTime, endTime
  ...
  const processVideo = async (video, crop) => {
    // Send video + crop coordinates + time range to backend for processing and OCR
    console.log(
      "[VSAllianceTab] Sending video to backend for processing with crop:",
      crop
    );
    ...
    try {
      // Create FormData to send video file, crop coordinates, and time range
      ...
      setProcessingStatus(
        `Extracted ${result.scoresRead} scores from ${result.framesProcessed} frames`
      );
      setProgress(100);
  ...
  const renderUploadStage = () => (
    <Box sx={{ width: "100%", mt: 2 }}>
      ...
      >
        ...
        />
        <label
          htmlFor={`${alliance}-video-upload-input`}
    ...
    <Box sx={{ width: "100%", mt: 2 }}>
      ...
      >
        <Typography variant="subtitle1" color="white" sx={{ mb: 1 }}>
          Total Score:{" "}
          <strong style={{ color: allianceColor }}>{totalScore}</strong>
        </Typography>
        <Typography variant="body2" color="text.secondary">
      ...
      >
        ...
        {scoreTimeline.map((entry, index) => (
          <Typography key={index} variant="body2" color="text.secondary">
            {entry.timestamp.toFixed(2)}s: Score {entry.score} (+
            {entry.increment})
          </Typography>
        ))}
      </Box>

      <Button variant="outlined" onClick={handleReset} sx={{ mt: 2 }} fullWidth>
