...
const getSupportedMimeType = () => {
  const mimeTypes = ["audio/mp4", "audio/m4a", "audio/aac", "audio/webm"];
...
function TimerPostmatch({
  data,
  cropStart: initialCropStart,
  setCropStart,
  cropEnd: initialCropEnd,
  setCropEnd,
  amplitudeData,
  audioBlob,
  detectedBuzzerTime,
  ...
  const renderAmplitudeGraph = () => {
    ...
    const maxTime = Math.max(
      amplitudeData[amplitudeData.length - 1]?.time || 0,
      1
    );
      ...
      <Box sx={{ width: "100%", overflow: "hidden" }}>
        ...
        >
          ...
          {amplitudeData.map((point, i) => {
            ...
            const isInCropRegion =
              point.time >= cropStart &&
              point.time <= cropStart + FRC_MATCH_DURATION;
          ...
          />
          <polygon
            points={`${(cropStart / maxTime) * canvasWidth},0 ${
              (cropStart / maxTime) * canvasWidth + 10
            },0 ${(cropStart / maxTime) * canvasWidth},10`}
            fill="#4CAF50"
          />
          ...
          <polygon
            points={`${
              ((cropStart + FRC_MATCH_DURATION) / maxTime) * canvasWidth
            },0 ${
              ((cropStart + FRC_MATCH_DURATION) / maxTime) * canvasWidth - 10
            },0 ${
              ((cropStart + FRC_MATCH_DURATION) / maxTime) * canvasWidth
            },10`}
            fill="#F44336"
    ...
    <Stack spacing={3}>
            ...
            <>
              ...
              <Box sx={{ mt: 3, px: 2 }}>
                <Typography variant="body2" sx={{ color: "white", mb: 1 }}>
                  ...
                  Start: <strong>{formatTime(cropStart)}</strong>
                  <Box component="span" sx={{ mx: 2 }}></Box>
                  <Box component="span" sx={{ color: "#F44336" }}>
                    ●
                  </Box>{" "}
                  End:{" "}
                  <strong>{formatTime(cropStart + FRC_MATCH_DURATION)}</strong>
                </Typography>

                {/* Crop Start Slider */}
                <Typography
                  variant="caption"
                  sx={{ color: "#4CAF50", display: "block", mb: 0.5 }}
                >
                ...
                <Typography variant="body2" sx={{ color: "white", mt: 2 }}>
                  Match will be: <strong>{formatTime(cropStart)}</strong> to{" "}
                  <strong>{formatTime(cropStart + FRC_MATCH_DURATION)}</strong>
                </Typography>
                <Typography
                  variant="caption"
      ...
      <Grid2 container spacing={1.5}>
        {[
          ...
          const isSelected = quickFeedback.includes(item.label);
          return (
            <Grid2 xs={6} sm={4} key={item.key}>
              <Button
                ...
                onClick={() => {
                  ...
                  let newFeedback;
                  if (isSelected) {
                    newFeedback = currentFeedback.filter(
                      (f) => f !== item.label
                    );
                  } else {
                    newFeedback = [...currentFeedback, item.label];
                  }
                  data.set(MatchStage.POST_MATCH, "quickFeedback", newFeedback);
                  update();
                }}
                fullWidth
                sx={{
                  ...
                  bgcolor: isSelected ? "rgba(255,152,0,0.8)" : "transparent",
                  textTransform: "none",
                  fontSize: "0.85rem",
                  py: 1.5,
                  "&:hover": {
...
function TimerContent({
  ...
  const detectBuzzer = (amplitudes) => {
    ...
    const avgAmplitude =
      amplitudes.reduce((sum, d) => sum + d.amplitude, 0) / amplitudes.length;
  ...
  const formatTimeRange = (start, end) => {
    return `${formatTime(start)} - ${formatTime(end)} (${(end - start).toFixed(
      1
    )}s)`;
  };
  ...
  return (
    <Stack spacing={3}>
      ...
      >
        <Typography
          variant="h6"
          sx={{ color: "rgba(255,255,255,0.7)", textAlign: "center", mb: 1 }}
        >
          Match Time
        </Typography>
        <Typography
        ...
        {currentShootStart !== null && (
          <Typography
            variant="body1"
            sx={{ color: "#FF9800", textAlign: "center", mt: 2 }}
          >
      ...
      {submittedTimes.length > 0 && (
        <Box sx={{ bgcolor: "rgba(0,0,0,0.6)", borderRadius: 2, p: 3 }}>
          ...
          >
            Total shooting time:{" "}
            {submittedTimes.reduce((sum, e) => sum + e.duration, 0).toFixed(1)}s
          </Typography>
        </Box>
      )}
      ...
      <Box sx={{ bgcolor: "rgba(0,0,0,0.4)", borderRadius: 2, p: 2 }}>
        ...
        >
          1. Click "Start Match" to begin
          <br />
          2. Press and hold the button to time each scoring action
          <br />
          3. Release to stop timing
          <br />
...
export default function TimerPage() {
  ...
  const handleNext = async () => {
    if (stage === MatchStage.PRE_MATCH) {
      ...
      const adjustedTimes = filteredTimes
        .map((time) => {
  ...
  useMemo(() => {
    switch (stage) {
      ...
      case MatchStage.GAMBLING:
        setCurrentComponent(
          <Gambling data={data} handleStageChange={handleStageChange} />
        );
        break;
      default:
        setCurrentComponent(<TimerPrematch data={data} />);
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  const quickFeedback =
                    data.get(MatchStage.POST_MATCH, "quickFeedback") || [];
                  const autoClimb =
                    data.get(MatchStage.TELEOP, "autoClimb") || "No climb";
                  const teleopClimb =
                    data.get(MatchStage.TELEOP, "teleopClimb") || "No climb";
                  // Get the adjusted shooting times (with crop applied)
                  const shootingTimes =
                    data.get(MatchStage.TELEOP, "shootingTimes") ||
                    submittedTimes;
                  ...
                  const relativeShootingTimes = shootingTimes
                    ...
                    .map((time) => ({
                      startShootTime: Math.max(
                        0,
                        time.startShootTime - cropStart
                      ),
                  ...
                  try {
                    ...
                    setAlert({
                      open: true,
                      message: "Timer data submitted successfully!",
                      severity: "success",
                    });
                    setTimeout(() => {
